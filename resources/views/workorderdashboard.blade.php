...
@section('content')
                    ...
                    <div class="p-6">
                        ...
                        @isset($totalWorkOrders)
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                                ...
                                <div class="rounded-lg border border-gray-200 p-4 bg-white">
                                    <div class="text-sm text-gray-500">Completed (Late)</div>
                                    <div class="mt-1 text-2xl font-semibold text-orange-700">{{ $completedLateWorkOrders ?? 0 }}
                                    </div>
                                </div>
                                <div class="rounded-lg border border-gray-200 p-4 bg-white">
                            ...
                            <div id="actions-toolbar" class="flex space-x-2" role="toolbar" aria-label="Toolbar Actions">
                                <div class="flex space-x-2" role="group" aria-label="Tambah WO Satuan">
                                    <a href="{{ route('work-orders.create') }}"
                                        class="no-underline bg-amber-500 hover:bg-amber-600 text-white px-3 py-2 rounded-full text-sm font-semibold shadow-sm hover:shadow ring-1 ring-black/5 transition active:scale-[.98]">➕
                                        Tambah WO
                                        (Satuan)</a>
                                </div>
                                ...
                                <div class="flex space-x-2" role="group" aria-label="Export Laporan">
                                    <button type="button" id="open-export-modal-btn"
                                        class="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-2 rounded-full rounded-pill-force text-sm font-semibold shadow-sm hover:shadow ring-1 ring-black/5 transition active:scale-[.98]"
                                        data-bs-toggle="modal" data-bs-target="#exportModal">📄 Export Laporan</button>
                                </div>
                            </div>
                        @endif
                    </div>
                        ...
                        <div class="p-4 mb-6 bg-gray-50 rounded-lg border border-gray-200">
                            <form action="{{ route('dashboard') }}" method="GET">
                                ...
                                <div class="mt-4 text-right">
                                    ...
                                    <button type="submit"
                                        class="appearance-none inline-flex items-center px-4 py-2 text-sm leading-5 font-semibold rounded-full text-white bg-gray-900 hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 shadow-sm hover:shadow ring-1 ring-black/5 transition active:scale-[.98]">
                                        Filter
                                    </button>
                                </div>
                            </form>
                        ...
                        <form id="bulk-actions-form" action="{{ route('work-orders.bulk-destroy') }}" method="POST">
                            ...
                            <div id="bulk-actions-toolbar" class="mb-4 hidden">
                                <div class="flex flex-wrap gap-3">
                                    <button type="button"
                                        class="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-full rounded-pill-force text-sm font-semibold shadow-sm hover:shadow ring-1 ring-black/5 transition active:scale-[.98]"
                                    ...
                                    <button type="button" id="delete-selected-btn"
                                        class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-full rounded-pill-force text-sm font-semibold shadow-sm hover:shadow ring-1 ring-black/5 transition active:scale-[.98]">🗑️
                                        Hapus yang
                                        Dipilih</button>
                                    <a href="#" id="export-selected-btn"
                                        class="no-underline bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-full text-sm font-semibold shadow-sm hover:shadow ring-1 ring-black/5 transition active:scale-[.98]">📄
                                        Export yang
                                        Dipilih</a>
                                </div>
                            </div>
    ...
    <div class="tw-modal hidden" id="exportModal" tabindex="-1" aria-labelledby="exportModalLabel" aria-hidden="true">
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 h-full w-full flex items-start justify-center z-9999">
                ...
                <form id="export-form" action="{{ route('work-orders.export') }}" method="GET">
                    ...
                    <div class="flex justify-end space-x-2 pt-4 border-t">
                        <a id="export-cancel-link" href="{{ route('dashboard') }}"
                            class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-full rounded-pill-force text-sm font-medium transition-colors">Batal</a>

                        <button type="submit"
                            class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-full rounded-pill-force text-sm font-medium transition-colors">Download
                            Laporan</button>
                    </div>
                </form>
            </div>
                ...
                <form id="bulk-edit-form" action="{{ route('work-orders.bulk-update-due-date') }}" method="POST">
                    ...
                    <div class="flex justify-end space-x-2 pt-4 border-t">
                        <button type="button" data-bs-dismiss="modal"
                            class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-full rounded-pill-force text-sm font-medium transition-colors">Batal</button>
                        <button type="submit"
                            class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-full rounded-pill-force text-sm font-medium transition-colors">Update
                            Due Date</button>
                    </div>
                </form>
            </div>
            ...
            function initDashboardScripts() {
                ...
                onReady(function() {
                    function tryInitMonthly() {
                        console.log('[Chart Debug] Work Order Dashboard tryInitMonthly called');
                        const canvas = document.getElementById('monthlyWoChart');
                        console.log('[Chart Debug] WO Canvas found:', !!canvas);
                        console.log('[Chart Debug] WO Chart available:', !!window.Chart);

                        if (!canvas || !window.Chart) {
                            console.log('[Chart Debug] WO Missing canvas or Chart');
                            return false;
                        }

                        // Prevent double initialization
                        if (canvas.dataset.chartInited === '1') {
                            console.log('[Chart Debug] WO Chart already initialized');
                            return true;
                        }

                        console.log('[Chart Debug] WO Fetching chart data...');
                        fetch("{{ route('charts.monthly-wo') }}")
                            .then(response => {
                                console.log('[Chart Debug] WO Fetch response:', response.status);
                                return response.json();
                            })
                            .then(data => {
                                console.log('[Chart Debug] WO Chart data received:', data);
                                let labels = data.labels || [];
                                let values = data.values || [];

                                // Only create dummy data if no real data exists
                                if (labels.length === 0 && values.length === 0) {
                                    console.log('[Chart Debug] WO No API data, creating dummy chart');
                                    labels = ['No Data Available'];
                                    values = [1];
                                } else {
                                    console.log('[Chart Debug] WO Using real API data - Labels:', labels
                                ...
                                console.log('[Chart Debug] WO Creating bar chart...');
                                try {
                                    ...
                                    console.log('[Chart Debug] WO Bar chart created successfully:', chart);
                                } catch (error) {
                            ...
                            .catch(err => {
                                ...
                                try {
                                    ...
                                    canvas.dataset.chartInited = '1';
                                    console.log('[Chart Debug] WO Error fallback chart created');
                                } catch (fallbackError) {
                ...
                rowCheckboxes.forEach(cb => cb.addEventListener('change', updateToolbarVisibility));
