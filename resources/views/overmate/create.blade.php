        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Tambah Overmate</h5>
                    <a href="{{ route('overmate.index') }}" class="btn btn-sm btn-outline-secondary">Kembali</a>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('overmate.store') }}">
                        @csrf
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="manufactur" class="form-control @error('manufactur') is-invalid @enderror" value="{{ old('manufactur') }}" required>
                            @error('manufactur')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Overmate Qty</label>
                            <input type="number" step="0.00001" name="overmate_qty" class="form-control @error('overmate_qty') is-invalid @enderror" value="{{ old('overmate_qty') }}" required>
                            @error('overmate_qty')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
